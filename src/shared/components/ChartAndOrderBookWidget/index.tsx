...
export const ChartAndOrderBookWidget = () => {
  ...
  const TAB_CONTENT_PROPS = {
    ...
    mainClassName: "relative z-0 border-t border-b border-divider-color-20 w-full flex justify-center items-center h-1/12",
  ...
  useEffect(() => {
    if (typeof window != "undefined") {
      ...
      setBlurHeight(window.innerHeight - (document.getElementById("orderform-mobile")?.offsetHeight || 0))
    }
  ...
  useEffect(() => {
    setBlurClassName(
      classnames("absolute top-0 w-screen blur bg-gray-500 bg-opacity-20 z-20 h-full webkitBlur", orderFormInFocus ? "block" : "hidden"),
    )

    setOrderformWrapperClassName(
      classnames("w-full z-20 bg-main-background-color h-auto", orderFormInFocus ? "bottom-16 fixed" : "bottom-0 pb-4"),
    )
  }, [orderFormInFocus])
  ...
  const renderTabContent = useCallback(
        ...
        case "Orderbook":
          ...
          return (
            <div style={{ height: orderbookHeight }}>
              <OrderBookWidget bottomSpace={515} isMobile={isVerticalMobileDevice || isHorizontalMobileDevice} />
            </div>
          )
        case "Chart":
          return (
    ...
    [selectedTab, orderbookHeight, isVerticalMobileDevice, isHorizontalMobileDevice],
  )
